...
- **Hero scene (GL path):** lit tower + ground-reflection pass, ring of 14 distant skyline buildings (kind 4) and a 120-point twinkling starfield (kind 5) placed at radius > camera distance so they never occlude the tower, pulsing aviation beacon (kind 3), per-window flicker, eased entrance, idle camera-tilt breathing, and a time-of-day atmosphere (`uNight` from the local hour drives sky colors, star/moon visibility, and skyline window density). Fragment-shader `vKind` branches must stay ordered high-to-low (moon 6 → stars 5 → skyline 4 → beacon 3 → ground 2).
