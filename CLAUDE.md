...
- `capm-ios.html` — iOS PWA build, regenerate via `python3 make_ios.py`. Injects iOS standalone-PWA head tags (data-URI touch icon + manifest) and refines the shared offline system-font aliases to iOS-native families.
...
Last validated content counts: QUIZ=220, CARDS=120, CONCEPTS=161, GLOSS=176.
...
- **Hero scene (GL path):** lit tower + ground-reflection pass, a restrained starfield, pulsing aviation beacon, per-window flicker, eased entrance, and idle camera breathing. Phase 9 removes the decorative skyline ring and uses a consistent architectural-daylight grade so the glass/mullions remain legible at 0% readiness. Fragment-shader `vKind` branches must stay ordered high-to-low (evergreen 8 → final beam 7 → moon 6 → stars 5 → skyline 4 → beacon 3 → ground 2). At readiness ≥99.5% the topping-out ceremony plays: a white final beam + evergreen (real construction tradition) descends onto the parapet, driven by the `uTop` uniform.
- **FX/UX layer:** particle bursts + confetti + streak float text + haptics on quiz answers, once-per-session splash intro, directional view slides keyed to tab order, tap ripples, and shareable canvas scorecards (`.sharebtn` on result screens, navigator.share with download fallback). Phase 9 removes the endlessly rotating CTA border and automatic device-tilt permission request in favor of quieter, purposeful motion.
- **Phase 8 visual system:** Blueprint Prism styling adds a technical grid, instrument-style hero HUD, richer native material depth, clearer section hierarchy, floating mobile dock, premium desktop navigation rail, refined charter presentation, and fully offline cross-platform system-font aliases.
- **Phase 9 visual system:** Architectural Intelligence is the current premium layer. It recomposes Home into a cinematic readiness-tower + Today command deck, creates independent desktop main/insight columns, introduces four custom inline-SVG domain illustrations, trims decorative borders/glows, increases mobile text contrast and touch targets, compacts the dock, and upgrades Quiz/Learn/Cards surfaces. The 10-question adaptive set is the first-run baseline. The final pre-INIT enhancement layer preserves per-tab scroll, sets `aria-current`, adds keyboard parity to the flashcard, and upgrades Settings to a focus-returning dialog with Escape/close handling and two-step reset confirmation.
...
Abhay learns best through quizzes, scenario-based questions, and PMBOK concepts explained with analogies from glazing/curtain-wall fabrication (floor QA checks, fabrication lines, spandrel panels, the One Yonge Toronto project). Phase 2c (complete) added EVM forecast drills (ETC, EAC variants, TCPI-against-EAC, EV-from-%-complete), quality-vs-grade, contract types, change control, critical path, and BA verification/validation & elicitation scenarios. Phase 2d (complete) rebalanced QUIZ toward d4 (elicitation technique selection, requirement types, product vs project scope, solution evaluation, weighted scoring, context diagrams) and d1 fundamentals (org structures, PMO types, phase gates, rolling wave, EEFs/OPAs). Scored-domain shares before Phase 2e were d1 29% / d2 19% / d3 29% / d4 23% vs exam weights 36/17/20/27. Phase 2e added 10 d1 and 6 d4 questions plus supporting cards, concepts, and glossary entries for PMI ethics, leadership vs. management, emotional intelligence, effective meetings, structured problem solving, and BA stakeholder-role distinctions. Domain d3 remains overweighted; continue favoring d1/d4 because append-only arrays cannot be rebalanced by removal.
